...
trait HandlesPricing {
	...
	protected function price_badge( array $choice ) {
		...
		if ( 'per_char' === $mode || 'per_char_nospace' === $mode ) {
			$suffix = '/' . esc_html__( 'char', 'simple-product-customizer' );
		} elseif ( 'per_word' === $mode ) {
			$suffix = '/' . esc_html__( 'word', 'simple-product-customizer' );
		} elseif ( 'per_unit' === $mode ) {
			$suffix = '/' . esc_html__( 'unit', 'simple-product-customizer' );
		}

		$show_pair = ( null !== $sale && $sale < $regular );

		if ( $show_pair ) {
			return '<span class="spcus-price-badge spcus-price-badge--has-sale">'
				. '<del class="spcus-price-badge__regular" aria-hidden="true">' . wp_kses_post( Money::html( $regular ) ) . '</del>'
				. ' <ins class="spcus-price-badge__sale">+' . wp_kses_post( Money::html( $sale ) ) . $suffix . '</ins>'
				. '</span>';
		}

		$amount = ( null !== $sale ) ? $sale : $regular;
		return '<span class="spcus-price-badge">+' . wp_kses_post( Money::html( $amount ) ) . $suffix . '</span>';
	}
}
