 ...
 *
 * One Controller owns a single `.spcus-options` wrapper (its product
 ...
 *
 * @param {HTMLElement} root `.spcus-options` element.
...
export default class Controller {
	 ...
	 */
	init() {
		if ( this.root.__spcusBound ) {
			return;
		}
		this.root.__spcusBound = true;
		...
		try {
			...
			this.recompute();
			this.root.classList.remove( 'spcus-loading' );
		} catch ( e ) {
	...
	/**
	 * Index every `.spcus-field` wrapper and prepare its logic + widgets.
	 *
	 * @return {void}
	 */
	discoverFields() {
		const els = this.root.querySelectorAll( '.spcus-field[data-field-id]' );
		els.forEach( ( el ) => {
	 ...
	 * `spcus_shipping_dynamics` hidden input + current product price.
	 *
	...
	shippingDynamics() {
		...
		try {
			const el = this.form
				? this.form.querySelector( '[name="spcus_shipping_dynamics"]' )
				: null;
			if ( el && el.value ) {
	...
	writeHiddenInputs( selections, linked, visibility ) {
		...
		set( 'spcus_field_data', JSON.stringify( payload ) );
		set( 'spcus_linked_products', JSON.stringify( linked ) );
	}
