...
final class DatetimeField extends AbstractField {
	...
	protected function inner() {
		...
		$cal_icon = '<span class="spcus-date__icon" aria-hidden="true">'
		...
		$clock_icon = '<span class="spcus-time__icon" aria-hidden="true">'
			. '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">'
			. '<circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.6"/>'
			. '<path d="M12 7.5V12l3 2" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>'
			. '</svg></span>';

		$date_input = '<input type="text" readonly class="spcus-input spcus-date-input" name="' . esc_attr( $this->input_name() ) . '_date"'
			. $this->attrs(
				array(
		...
		$time_input = '<input type="text" readonly class="spcus-input spcus-time-input" name="' . esc_attr( $this->input_name() ) . '_time"'
			. $this->attrs(
				array(
		...
		$html  = '<div class="spcus-datetime" data-variant="datetime" data-field-id="' . esc_attr( $this->id() ) . '"'
			. $this->attrs( $this->choice_price_attrs( $choice ) ) . '>';
		$html .= '<div class="spcus-datetime__part spcus-date">' . $cal_icon . $date_input . '</div>';
		$html .= '<div class="spcus-datetime__part spcus-time">' . $clock_icon . $time_input . '</div>';
		$html .= '</div>';
		return $html;
	}
