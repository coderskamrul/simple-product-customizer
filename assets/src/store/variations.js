 ...
 * percent-base from the `#spcus-variation-prices` and
 * `#spcus-variation-prices-pct` holder maps (keyed by variation id), then
...
export function readBase( root ) {
	const baseEl = root.querySelector( '#spcus-base-price' );
	const pctEl = root.querySelector( '#spcus-base-price-pct' );
	const num = ( el ) =>
...
export function initVariations( root, form, apply ) {
	...
	const onFound = ( _e, variation ) => {
		...
		const prices = holderMap( root, 'spcus-variation-prices' );
		const pcts = holderMap( root, 'spcus-variation-prices-pct' );
		const base =
