	...
	public function tokenize( string $src ): array {
		...
		while ( $i < $len ) {
			...
			if ( '[' === $ch ) {
				...
				if ( $i >= $len || ']' !== $src[ $i ] ) {
					throw new EvaluationError( 'Unclosed variable placeholder at position ' . esc_html( $start ) );
				}
				++$i; // Consume the closing ].
				$name = trim( $name );
				if ( '' === $name ) {
					throw new EvaluationError( 'Empty variable placeholder at position ' . esc_html( $start ) );
				}
			...
			if ( ctype_digit( $ch ) || '.' === $ch ) {
				...
				if ( '.' === $number || '' === $number ) {
					throw new EvaluationError( 'Invalid number at position ' . esc_html( $start ) );
				}
			...
			throw new EvaluationError( 'Unexpected character "' . esc_html( $ch ) . '" at position ' . esc_html( $i ) );
		}
