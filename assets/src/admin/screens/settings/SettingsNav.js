...
export default function SettingsNav( { active, onSelect } ) {
	...
	return (
		<nav className="spcus-set-nav" aria-label="Settings sections">
			<ul className="spcus-set-nav__list">
				{ SECTIONS.map( ( s, i ) => {
					...
					return (
						<li key={ s.id }>
							<button
								...
								className={ classNames( 'spcus-set-nav__item', {
									'is-active': isActive,
								} ) }
							...
							>
								...
								<span className="spcus-set-nav__text">
									<span className="spcus-set-nav__title">
										{ s.title }
									</span>
									<span className="spcus-set-nav__sub">
										{ s.nav }
									</span>
								</span>
								<span
