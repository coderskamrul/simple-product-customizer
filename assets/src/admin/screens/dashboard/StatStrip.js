...
function TrendChip( { value } ) {
	...
	const up = value >= 0;
	return (
		<span className={ `spcus-db-chip spcus-db-chip--${ up ? 'up' : 'down' }` }>
			<span
...
function StatTile( { tone, icon, label, value, chip } ) {
	return (
		<div className="spcus-db-stat">
			<span
				className={ `spcus-db-stat__icon spcus-db-stat__icon--${ tone }` }
				aria-hidden="true"
			>
				<span className={ `dashicons dashicons-${ icon }` } />
			</span>
			<div className="spcus-db-stat__body">
				<div className="spcus-db-stat__top">
					<span className="spcus-db-stat__value">{ value }</span>
					{ chip }
				</div>
				<span className="spcus-db-stat__label">{ label }</span>
			</div>
		</div>
	);
}
	...
	const cards = [
		{
			...
			chip: publishedCount > 0 && (
				<span className="spcus-db-chip spcus-db-chip--neutral">
		...
		},
		{
			...
			chip: deltas.revenueAbs !== 0 && (
				<span
					className={ `spcus-db-chip spcus-db-chip--${
						revUp ? 'up' : 'down'
					}` }
				>
					<span
