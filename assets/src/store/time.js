 ...
 *
 * Turns the readonly `.spcus-time-input` rendered by TimeField.php into a
...
export function wireTime( fieldEl, onChange ) {
	const input = fieldEl.querySelector( '.spcus-time-input' );
	if ( ! input || input.__spcusTime ) {
		return;
	}
	input.__spcusTime = true;
