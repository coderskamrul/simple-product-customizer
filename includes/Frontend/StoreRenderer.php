 ...
 * store JS bundle (window.spcusStore) consumes everything written here, so the
...
final class StoreRenderer {
	...
	public function render() {
		...
		foreach ( $set_ids as $set_id ) {
			...
			$rendered_sets[] = sprintf(
				'<div class="spcus-set" data-set-id="%d">%s</div>',
				$set_id,
				$inner
			);
			...
			do_action( 'spcus_stats_record', $set_id, 'impressions', 0 );
		}
		...
		do_action( 'spcus_enqueue_store_assets', $product_id, $published_ids );

		$html  = sprintf(
			'<div class="spcus-options spcus-loading" data-product-id="%d">',
			$product_id
		);
		$html .= '<div class="spcus-loader" aria-hidden="true"></div>';
	...
	private function hidden_inputs( $product, array $published_ids ) {
		...
		$html  = '<input type="hidden" name="spcus_field_data" class="spcus-field-data" value="" />';
		$html .= '<input type="hidden" name="spcus_linked_products" class="spcus-linked-products" value="" />';
		$html .= sprintf(
			'<input type="hidden" name="spcus_published_set_ids" value="%s" />',
			esc_attr( (string) wp_json_encode( array_values( array_map( 'intval', $published_ids ) ) ) )
		);
		$html .= sprintf(
			'<input type="hidden" name="spcus_shipping_dynamics" value="%s" />',
			esc_attr( (string) wp_json_encode( $shipping ) )
		);

		$html .= sprintf(
			'<span class="spcus-holder" id="spcus-base-price" data-value="%s"></span>',
			esc_attr( (string) $base )
		);
		$html .= sprintf(
			'<span class="spcus-holder" id="spcus-base-price-pct" data-value="%s"></span>',
			esc_attr( (string) $base_pct )
		);
		$html .= sprintf(
			'<span class="spcus-holder" id="spcus-variation-prices" data-value="%s"></span>',
			esc_attr( (string) wp_json_encode( $variation_prices ) )
		);
		$html .= sprintf(
			'<span class="spcus-holder" id="spcus-variation-prices-pct" data-value="%s"></span>',
			esc_attr( (string) wp_json_encode( $variation_prices_pct ) )
		);
		$html .= sprintf(
			'<span class="spcus-holder" id="spcus-product-attributes" data-product-type="%s" data-value="%s"></span>',
	...
	private function price_summary( $product ) {
		...
		$out = '<div class="spcus-price-summary">';

		if ( $show_price ) {
			$out .= sprintf(
				'<div class="spcus-price-row"><strong class="spcus-price-label">%s</strong> <span id="spcus-options-price" class="spcus-price-value">%s</span></div>',
				esc_html( (string) $settings->get( 'priceLineLabel', __( 'Options Price', 'simple-product-customizer' ) ) ),
		...
		if ( $show_total ) {
			$out .= sprintf(
				'<div class="spcus-price-row"><strong class="spcus-price-label">%s</strong> <span id="spcus-options-total" class="spcus-price-value">%s</span></div>',
				esc_html( (string) $settings->get( 'totalLineLabel', __( 'Total Price', 'simple-product-customizer' ) ) ),
