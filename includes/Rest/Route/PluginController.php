 ...
 * `manage_options` plus the `pkitfw_rest` nonce.
 */
final class PluginController {
	...
	private function install( WP_REST_Request $r, RestServer $s ) {
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			return $s->fail( 'forbidden', __( 'You are not allowed to install plugins.', 'productkit-for-woocommerce' ), 403 );
		}

		$slug = sanitize_key( (string) $r->get_param( 'slug' ) );
		if ( '' === $slug ) {
			return $s->fail( 'bad_slug', __( 'Missing plugin slug.', 'productkit-for-woocommerce' ), 400 );
		}
		...
		if ( $installed ) {
			...
			return $s->ok(
				array(
					'message' => __( 'Plugin activated.', 'productkit-for-woocommerce' ),
					'slug'    => $slug,
				)
			);
		}
		...
		if ( ! class_exists( 'Plugin_Upgrader' ) || ! class_exists( 'WP_Ajax_Upgrader_Skin' ) ) {
			return $s->fail( 'no_upgrader', __( 'Plugin installer unavailable.', 'productkit-for-woocommerce' ), 500 );
		}
		...
		if ( is_wp_error( $api ) || empty( $api->download_link ) ) {
			return $s->fail( 'api_failed', __( 'Could not fetch plugin information.', 'productkit-for-woocommerce' ), 500 );
		}
		...
		if ( true !== $result ) {
			return $s->fail( 'install_failed', __( 'Plugin installation failed.', 'productkit-for-woocommerce' ), 500 );
		}
		...
		return $s->ok(
			array(
				'message' => __( 'Plugin installed and activated.', 'productkit-for-woocommerce' ),
				'slug'    => $slug,
			)
		);
	}
