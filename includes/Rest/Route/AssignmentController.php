...
final class AssignmentController {
	...
	const OPT_IMAGE_MAP = 'spcus_product_image_map';
	...
	private function get_assignment( WP_REST_Request $r, RestServer $s ) {
		$repo = $this->repo();
		if ( ! $repo ) {
			return $s->fail( 'unavailable', __( 'Storage unavailable.', 'simple-product-customizer' ), 500 );
		}
	...
	private function save_assignment( WP_REST_Request $r, RestServer $s ) {
		...
		if ( ! $resolver ) {
			return $s->fail( 'unavailable', __( 'Assignment resolver unavailable.', 'simple-product-customizer' ), 500 );
		}

		$set_id = (int) $r->get_param( 'set_id' );
		if ( $set_id <= 0 ) {
			return $s->fail( 'bad_id', __( 'Missing set id.', 'simple-product-customizer' ), 400 );
		}
	...
	private function first_product_link( $scope, array $include, array $exclude ) {
		...
		$query_args = array(
			...
			'posts_per_page' => 1,
			'fields'         => 'ids',
			// phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in -- bounded existence probe (posts_per_page => 1, fields => ids) over an admin-authored exclusion list.
		...
		if ( isset( $tax_map[ $scope ] ) && ! empty( $include ) ) {
			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- single bounded admin-only assignment lookup, not a front-end query.
