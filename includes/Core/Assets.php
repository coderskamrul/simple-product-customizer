...
final class Assets {
	...
	public static function manifest( $handle_base, array $fallback_deps = array() ) {
		$asset_file = SPCUS_PATH . 'assets/build/' . $handle_base . '.asset.php';
		if ( is_readable( $asset_file ) ) {
			$data = include $asset_file;
			return array(
				'deps'    => isset( $data['dependencies'] ) ? $data['dependencies'] : $fallback_deps,
				'version' => isset( $data['version'] ) ? $data['version'] : SPCUS_VERSION,
				'exists'  => true,
			);
		}
		return array(
			...
			'exists'  => is_readable( SPCUS_PATH . 'assets/build/' . $handle_base . '.js' ),
		);
	}
	...
	public static function script( $handle, $base, array $fallback = array(), $in_footer = true ) {
		...
		wp_enqueue_script( $handle, SPCUS_ASSETS . $base . '.js', $m['deps'], $m['version'], $in_footer );
		return true;
	}
	...
	public static function style( $handle, $base ) {
		$file = SPCUS_PATH . 'assets/build/' . $base . '.css';
		if ( is_readable( $file ) ) {
			...
			$version = $version ? (string) $version : SPCUS_VERSION;
			wp_enqueue_style( $handle, SPCUS_ASSETS . $base . '.css', array(), $version );
		}
	}
}
