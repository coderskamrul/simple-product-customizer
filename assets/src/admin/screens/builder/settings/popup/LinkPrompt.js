...
export default function LinkPrompt( {
	...
	const newTabId = useId( 'spcus-rte-newtab' );
	...
	return (
		<div className="spcus-rte__linkbar">
			<input
				ref={ inputRef }
				type="url"
				className="spcus-rte__linkinput"
				placeholder={ __(
					'https://example.com',
			...
			/>
			<label className="spcus-rte__linkcheck" htmlFor={ newTabId }>
				<input
					id={ newTabId }
					type="checkbox"
					checked={ newTab }
				...
				/>
				{ __( 'New tab', 'simple-product-customizer' ) }
			</label>
			<button
				type="button"
				className="spcus-rte__btn"
				title={ __(
					'Apply link',
					'simple-product-customizer'
				) }
			...
			{ hasLink && (
				<button
					type="button"
					className="spcus-rte__btn is-danger"
					title={ __(
						'Remove link',
						'simple-product-customizer'
					) }
