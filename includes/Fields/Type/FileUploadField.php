...
final class FileUploadField extends AbstractField {
	...
	private function hint( $key, $token, $value ) {
		...
		return '<span class="spcus-upload__hint">' . esc_html( $text ) . '</span>';
	}
	...
	protected function inner() {
		$choices = $this->choices();
		$choice  = isset( $choices[0] ) && is_array( $choices[0] ) ? $choices[0] : array();
		$uid     = 'spcus-upload-' . $this->id();

		$upload_text = (string) $this->cfg( 'uploadText', '' );
		$upload_text = '' !== $upload_text ? $upload_text : __( 'Upload', 'simple-product-customizer' );
		$drag_text   = (string) $this->cfg( 'dragText', '' );
		$drag_text   = '' !== $drag_text ? $drag_text : __( 'Click or drag and drop', 'simple-product-customizer' );

		$max_size = '' !== (string) $this->cfg( 'maxSize', '' ) ? (int) $this->cfg( 'maxSize' ) : '';
		$max_num  = '' !== (string) $this->cfg( 'maxNumber', '' ) ? (int) $this->cfg( 'maxNumber' ) : '';

		$html  = '<div class="spcus-upload" data-field-id="' . esc_attr( $this->id() ) . '"'
			. $this->attrs( $this->choice_price_attrs( $choice ) ) . '>';
		$html .= '<input type="hidden" class="spcus-upload__data" name="' . esc_attr( $this->input_name() ) . '" value="" />';
		$html .= '<label class="spcus-dropzone" for="' . esc_attr( $uid ) . '">';
		$html .= '<span class="spcus-dropzone__btn"><svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 16V4M6 10l6-6 6 6M4 18v2h16v-2"/></svg>' . esc_html( $upload_text ) . '</span>';
		$html .= '<span class="spcus-dropzone__text">' . esc_html( $drag_text ) . '</span>';
		$html .= '<input type="file" id="' . esc_attr( $uid ) . '" class="spcus-upload__input" multiple'
			. $this->attrs(
				array(
					'accept'          => $this->accept(),
		...
		$html .= '</label>';
		$html .= '<div class="spcus-upload__progress" hidden><span class="spcus-upload__bar"></span></div>';
		$html .= '<div class="spcus-upload__result"></div>';
		...
		if ( '' !== $hints ) {
			$html .= '<div class="spcus-upload__hints">' . $hints . '</div>';
		}
