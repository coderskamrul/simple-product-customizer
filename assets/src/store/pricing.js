/**
 * Client-side option pricing — mirrors SPCUS\Pricing\PriceCalculator (§13).
 *
...
function choiceElements( fieldEl ) {
	const opts = fieldEl.querySelectorAll( '.spcus-select__opt[data-index]' );
	if ( opts.length ) {
		const out = [];
		opts.forEach( ( o ) => {
	...
	Object.keys( selections ).forEach( ( fieldId ) => {
		...
		if ( entry.type === 'select' ) {
			...
			const opt = el.querySelector(
				'.spcus-select__opt[data-index="' + indexes[ 0 ] + '"]'
			);
			if ( ! opt ) {
				return;
			}
...
export function priceFormula(
	...
	const node = fieldEl.querySelector( '.spcus-formula' );
	if ( ! node ) {
		return 0;
	}
...
/**
 * Render the price spans (#spcus-options-price / #spcus-options-total).
...
export function renderPriceSpans( root, optionsPrice, basePrice, linkedPrice ) {
	const priceEl = root.querySelector( '#spcus-options-price' );
	const totalEl = root.querySelector( '#spcus-options-total' );
	if ( priceEl ) {
